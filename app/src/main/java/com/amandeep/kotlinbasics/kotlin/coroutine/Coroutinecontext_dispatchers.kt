...
fun main() = runBlocking {
    ...
    launch(Dispatchers.Unconfined) {
        ...
        //will get the last executed context that is some other thread.
        launch(coroutineContext) {
            println("C4: ${Thread.currentThread().name}")       // Thread: some other thread T1
            delay(1000)
            println("C4 after delay: ${Thread.currentThread().name}")   // Thread: some other thread T1
        }
    }
